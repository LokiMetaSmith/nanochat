...
if compile:
    print0(f"compiling the model (dynamic={compile_dynamic})...")
    model = torch.compile(model, dynamic=compile_dynamic) # the inputs to model will never change shape so dynamic=False is safe
